 ...
 $create_ticket = {
  ...
  elseif($checker -eq -1)
  {
        ...
        $JSON = @{
            ...
            "title"    = "Known Bad IP is logging into $dealerid"
            "text"     = "catchfraud.ps1"
            "sections" = @(
            @{
...
#################################################################################
# Gather the IP Addresses that have been issued authentication in the last hour #
#################################################################################
$ipcount = Invoke-RestMethod -method Get -Uri http://10.3.55.16:9000/api/search/universal/relative/terms?field=hostAddress"&"query=context:issued-authentication"&"size=1750"&"range=10000"&"filter=streams%3A5ac3c2db3756a80b9015c611 -Headers $Headers
...
# Start iterating through each IP address and query Postgres database for known bad IPAddresses    
foreach($ipaddress in $ips){
    
    Write-Output $ipaddress  
    $DBCmd = $DBConn.CreateCommand();
    $DBCmd.CommandText = "SELECT * FROM fraud_ips WHERE Address = '$ipaddress';";
    $badlogin = $DBCmd.ExecuteScalar();
    Write-Output $badlogin
    ...
    # If the query returned a result start evaluation and create a ticket for Jira
    Else{
        Write-Output $ipaddress
        Start-Sleep -Seconds 2

        # Grab the dealerid for the positive query result
        $dealerid = Invoke-Sqlcmd -Query "select top 50 a.DateCreate, a.username, a.wasSuccessful, b.dealerid, a.IPAddress from LoginAttempt a inner join dealeruser b on a.DealerUserID = b.UserID where ipaddress in('$ipaddress')  and a.wasSuccessful in ('True') order by loginid desc" -ServerInstance $Database | Select-Object dealerid
        Write-Output $dealerid
        $dealerid = $dealerid.dealerid | Sort-Object -Unique
        Write-Output $dealerid
        Start-Sleep -Seconds 2
        # Check for an IP that has authenticated to multiple dealer accounts
        $dealerid_count = $dealerid | Measure-Object -Line | Select-Object -ExpandProperty Lines
        Write-Output $dealerid_count
        Start-Sleep -Seconds 2

        # If there are multiple dealerids, iterate through each one
        If($dealerid_count -gt 1){
            Write-Output "multiple ids"
            $dealerids = $dealerid
            foreach($dealerid in $dealerids){
                #$dealerid = $dealerid.dealerid
                # First check to see if the login is old
                $ipDealerHistoryCheck = Invoke-Sqlcmd -Query "select top 100 a.DateCreate, a.IPAddress, b.dealerid, a.wasSuccessful from LoginAttempt a inner join dealeruser b on a.DealerUserID = b.UserID where IPAddress in('$ipaddress') AND dealerid in('$dealerid') AND a.DateCreate < DATEADD(DAY, -2, GETDATE()) AND a.wasSuccessful in ('True') order by loginid desc" -ServerInstance $Database

                if($ipDealerHistoryCheck){
                    Write-Output "its old"
                    Write-Output $dealerid
                    write-host $ipDealerHistoryCheck
                    Start-Sleep -Seconds 3
                }
                elseif(!$ipDealerHistoryCheck){
                    Write-Output "its new"
                    Write-Output $dealerid
                    start-sleep -seconds 3
                    write-host $ipDealerHistoryCheck
                    &$start_ticket
                    ...
                    Write-Output "no history evaluated"
                }
            }
        }
        Elseif($dealerid_count -eq 1){
            Write-Output "single id"
            $ipDealerHistoryCheck = Invoke-Sqlcmd -Query "select top 100 a.DateCreate, a.IPAddress, b.dealerid, a.wasSuccessful from LoginAttempt a inner join dealeruser b on a.DealerUserID = b.UserID where IPAddress in('$ipaddress') AND dealerid in('$dealerid') AND a.DateCreate < DATEADD(DAY, -2, GETDATE()) AND a.wasSuccessful in ('True') order by loginid desc" -ServerInstance $Database
