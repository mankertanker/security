 ...
 $create_ticket = {
  ...
  elseif($checker -eq -1)
  {
        ...
        $JSON = @{
            ...
            "title"    = "Private Sale $dealerid"
            "text"     = "private_sale.ps1"
            "sections" = @(
            @{
...
$ipcount = Invoke-RestMethod -method Get -Uri https://graylog-cluster.carsforsale.local:443/api/search/universal/relative/terms?field=rawjson"&"query=ApiCall%3AaddNewInventory"&"size=200"&"range=4000"&"filter=streams%3A5978df67041f63048180339b -Headers $Headers
$info_used = $ipcount
...
$run = $decode_data 
$desc = $run.split('&') | Select-String -Pattern "Description="


$private = $desc | Select-String -Pattern "\bprivate.seller\b"
if (-not ([string]::IsNullOrEmpty($private))){
echo "Private Seller $dealerid $private!!"

        Echo "Start Ticket"
        &$start_ticket
}
$sale = $desc | Select-String -Pattern "\bprivate.sale\b"
if (-not ([string]::IsNullOrEmpty($sale))){
echo "Private Sale $dealerid $sale!!"

        Echo "Start Ticket"
        &$start_ticket
}
$flow = $desc | Select-String -Pattern "\bThis.is.a.test\b"
if (-not ([string]::IsNullOrEmpty($flow))){
echo "Private Sale $dealerid $flow!!"

        Echo "Start Ticket"
        &$start_ticket
}
$payment = $desc | Select-String -Pattern "\badvanced.payment\b"
if (-not ([string]::IsNullOrEmpty($payment))){
echo "Private Sale $dealerid $payment!!"

        Echo "Start Ticket"
        &$start_ticket
}
$payments = $desc | Select-String -Pattern "\badvance.payment\b"
if (-not ([string]::IsNullOrEmpty($payments))){
echo "Private Sale $dealerid $payments!!"

        Echo "Start Ticket"
        &$start_ticket
}
$iam = $desc | Select-String -Pattern "\bI.am.selling\b"
if (-not ([string]::IsNullOrEmpty($iam))){
echo "Private Sale $dealerid $iam!!"

        Echo "Start Ticket"
        &$start_ticket
}
<#$cash = $desc | Select-String -Pattern "\bcash.only\b"
if (-not ([string]::IsNullOrEmpty($cash))){
echo "Private Sale $dealerid $cash!!"

        Echo "Start Ticket"
        &$start_ticket
}#>
$auction = $desc | Select-String -Pattern "\bauction123\b"
if (-not ([string]::IsNullOrEmpty($auction))){
echo "Private Sale $dealerid $auction!!"

        Echo "Start Ticket"
        &$start_ticket
}
$gmail = $desc | Select-String -Pattern "\bgmail.c()m\b"
if (-not ([string]::IsNullOrEmpty($gmail))){
echo "Private Sale $dealerid $gmail!!"

        Echo "Start Ticket"
        &$start_ticket
}
$me = $desc | Select-String -Pattern "\bfor.more.info.contact.me.directly.at\b"
if (-not ([string]::IsNullOrEmpty($me))){
echo "Private Sale $dealerid $me!!"

        Echo "Start Ticket"
        &$start_ticket
}
$very = $desc | Select-String -Pattern "\bi.am.very.negotiable\b"
if (-not ([string]::IsNullOrEmpty($very))){
echo "Private Sale $dealerid $me!!"
